"""Train an Amortized IRT (3PL, sentence-encoder) model.

Run from starting_kit/:  python amortized_3pl_sentence_submission/train.py
"""
...
print(f"Final loss — amortized IRT (3PL, sentence): {history['losses'][-1]:.4f}")

est_ability = model.ability.detach()
print(f"Mean ability: {est_ability.mean():.4f}, std: {est_ability.std():.4f}")

save_path = os.path.join(_dir, "amortized_3pl_sentence.pt")
torch.save(
    {
