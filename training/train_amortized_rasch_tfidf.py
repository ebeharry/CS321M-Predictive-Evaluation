"""Train an Amortized IRT (1PL/Rasch, TF-IDF) model.

Run from starting_kit/:  python amortized_rasch_tfidf_submission/train.py
"""
...
print(f"Final loss — amortized IRT (Rasch, TF-IDF): {history['losses'][-1]:.4f}")

est_ability = model.ability.detach()
print(f"Mean ability: {est_ability.mean():.4f}, std: {est_ability.std():.4f}")

save_path = os.path.join(_dir, "amortized_rasch_tfidf.pt")
torch.save(
    {
