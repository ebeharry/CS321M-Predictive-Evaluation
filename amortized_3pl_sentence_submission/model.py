"""Amortized IRT (3PL, sentence-encoder) submission for the Predictive AI Evaluation Challenge."""
...
_ckpt = torch.load(
    os.path.join(_dir, "amortized_3pl_sentence.pt"),
    weights_only=True,
    map_location="cpu",
)
...
print(f"AmortizedIRT (3PL, sentence) loaded: {_ckpt['n_subjects']} subjects, {_ckpt['n_items']} items, encoder={_ckpt['encoder_name']}")
...
def predict(input: dict, labeled=None) -> float:
    ...
    with torch.no_grad():
        ...
        c = torch.sigmoid(params[0, 2])

    s = _subj_to_idx.get(input["subject_content"])
    theta = _ability[s] if s is not None else _mean_ability

    prob = c + (1 - c) * torch.sigmoid(a * (theta - b))
    return torch.clamp(prob, 0.001, 0.999).item()
